...
public class AiTopicsActivity extends AppCompatActivity {
    ...
    MainActivity k;
    String calll;
    ArrayList<AiTopicHelperClass> fewAllLocatio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        calll = getIntent().getStringExtra("vacai");
        ...
        if (calll != null && calll.equals("vac")) {
            for (int i = 0; i < k.videoIds.size(); i++) {
                if (k.course_id.get(i) == 1)
                    fewAllLocatio.add(new AiTopicHelperClass(aitopics.get(k.topic.get(i)), k.vtitle.get(i), k.authorr.get(i), R.drawable.ai_logo, R.drawable.afepluslot_l));
            }
        }
        if (getcoursepos != null && getcoursepos.equals(String.valueOf(1))) {
            for (int i = 0; i < k.videoIds.size(); i++) {
                if (k.topic.get(i) == Integer.parseInt(getadaptpos) && k.course_id.get(i) == Integer.parseInt(getcoursepos))
                    fewAllLocatio.add(new AiTopicHelperClass(aitopics.get(Integer.parseInt(getadaptpos)), k.vtitle.get(i), k.authorr.get(i), R.drawable.ai_logo, R.drawable.afepluslot_l));
            }
        }

        adapter4t = new AiTopicAdapter(fewAllLocatio);
