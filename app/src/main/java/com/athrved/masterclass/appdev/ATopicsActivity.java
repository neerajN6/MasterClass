...
public class ATopicsActivity extends AppCompatActivity {
    ...
    MainActivity k;
    String calll;
    ArrayList<ATopicHelperClass> fewAllLocatio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        calll = getIntent().getStringExtra("vacapp");
        ...
        if (calll != null && calll.equals("vac")) {
            for (int i = 0; i < k.videoIds.size(); i++) {
                if (k.course_id.get(i) == 3)
                    fewAllLocatio.add(new ATopicHelperClass(apptopics.get(k.topic.get(i)), k.vtitle.get(i), k.authorr.get(i), R.drawable.ai_logo, R.drawable.afepluslot_l));
            }
        }
        if (getcoursepos != null && getcoursepos.equals(String.valueOf(3))) {
            for (int i = 0; i < k.videoIds.size(); i++) {
                if (k.topic.get(i) == Integer.parseInt(getadaptpos) && k.course_id.get(i) == Integer.parseInt(getcoursepos))
                    fewAllLocatio.add(new ATopicHelperClass(apptopics.get(Integer.parseInt(getadaptpos)), k.vtitle.get(i), k.authorr.get(i), R.drawable.ai_logo, R.drawable.afepluslot_l));
            }
        }

        adapter4t = new ATopicAdapter(fewAllLocatio);
