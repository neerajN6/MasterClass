...
public class UiuxTopicsActivity extends AppCompatActivity {
    ...
    MainActivity k;
    String calll;
    ArrayList<UiuxTopicHelperClass> fewAllLocatio;
        ...
        if (getcoursepos != null && getcoursepos.equals(String.valueOf(5))) {
            for (int i = 0; i < k.videoIds.size(); i++) {
                if (k.topic.get(i) == Integer.parseInt(getadaptpos) && k.course_id.get(i) == Integer.parseInt(getcoursepos))
                    fewAllLocatio.add(new UiuxTopicHelperClass(uiuxtopics.get(Integer.parseInt(getadaptpos)), k.vtitle.get(i), k.authorr.get(i), R.drawable.ai_logo, R.drawable.afepluslot_l));
            }
        }

        adapter4t = new UiuxTopicAdapter(fewAllLocatio);
