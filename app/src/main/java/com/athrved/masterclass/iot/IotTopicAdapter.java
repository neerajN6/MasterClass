...
public class IotTopicAdapter extends RecyclerView.Adapter<IotTopicAdapter.IotTopicViewHolder> {

    VideoData vv;

    ArrayList<IotTopicHelperClass> featloc;

    public IotTopicAdapter(ArrayList<IotTopicHelperClass> featloc) {
        this.featloc = featloc;
    }

    @NonNull
    @Override
    public IotTopicViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.add_all_classes, parent, false);
        IotTopicViewHolder IotTopicViewHolder = new IotTopicViewHolder(view);


        return IotTopicViewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull IotTopicViewHolder holder, int position) {
        IotTopicHelperClass IotTopicHelperClass = featloc.get(position);
        holder.ffimagebig.setImageResource(IotTopicHelperClass.getItimagebig());
        holder.ffimagesmall.setImageResource(IotTopicHelperClass.getItimagesmall());
        holder.fftitle.setText(IotTopicHelperClass.getIttitle());
        holder.fftopic.setText(IotTopicHelperClass.getItopicname());
        holder.ffauthor.setText(IotTopicHelperClass.getItauthor());

        MainActivity k = new MainActivity();
        k.getdatatotop();
        IotActivity g = new IotActivity();
        int gh = g.callla;
        IotTopicsActivity ta = new IotTopicsActivity();
        ...
        for (int pk = 0; pk < k.videoIds.size(); pk++)
            if (k.course_id.get(pk) == 6 && k.topic.get(pk) == ll)
                count1++;
        ...
        for (int i = 0; i < count1; i++) {
            if (holder.getAdapterPosition() == i) {
                ...
                for (int ki = 0; ki < k.videoIds.size(); ki++) {
                    if (k.course_id.get(ki) == 6 && k.topic.get(ki) == ll) {
...
//        }
    ...
    public static class IotTopicViewHolder extends RecyclerView.ViewHolder{
        ImageView ffimagebig, ffimagesmall;
        TextView fftitle, fftopic, ffauthor;
        Button bookmark, bookmark_border;

        public IotTopicViewHolder(@NonNull final View itemView){
            super(itemView);
            ...
            IotTopicsActivity l = new IotTopicsActivity();
