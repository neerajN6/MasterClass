...
public class IotTopicsActivity extends AppCompatActivity {
    ...
    MainActivity k;
    String calll;
    ArrayList<IotTopicHelperClass> fewAllLocatio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        calll = getIntent().getStringExtra("vaciot");
...
//            }
//        }
        if (getcoursepos != null && getcoursepos.equals(String.valueOf(6))) {
            for (int i = 0; i < k.videoIds.size(); i++) {
                if (k.topic.get(i) == Integer.parseInt(getadaptpos) && k.course_id.get(i) == Integer.parseInt(getcoursepos))
                    fewAllLocatio.add(new IotTopicHelperClass(iottopics.get(Integer.parseInt(getadaptpos)), k.vtitle.get(i), k.authorr.get(i), R.drawable.ai_logo, R.drawable.afepluslot_l));
            }
        }

        adapter4t = new IotTopicAdapter(fewAllLocatio);
